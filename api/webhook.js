...
// Обработчик для команды /q
async function handleQCommand(chatId, messageId, bot, text, botUsername, questions) {
...
function handleRulesCommand(chatId, messageId, bot) {
  // Логика для команды /rules
}

// Обработчик для команды /stop11
function handleStop11Command(chatId, messageId, bot) {
  // Логика для команды /stop11
}

// Обработчик для команды /stop10
function handleStop10Command(chatId, messageId, bot) {
  // Логика для команды /stop10
}
  ...
  try {
    ...
    if (body.message) {
      ...
      switch (text) {
				case '/q':
				case `/q@${botUsername}`:
					handleQCommand(id, message_id, bot)
					break
				case '/rules':
				case `/rules@${botUsername}`:
					handleRulesCommand(id, message_id, bot)
					break
				case '/stop11':
				case `/stop11@${botUsername}`:
					handleStop11Command(id, message_id, bot)
					break
				case '/stop10':
				case `/stop10@${botUsername}`:
					handleStop10Command(id, message_id, bot)
					break
				// Другие команды
			}
...
// ✅  Разрешено: 
// делиться только опытом разбора по 10 шагу. 
